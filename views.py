...
def login(request):
    ...
    return render(request, 'nmap_auth.html', r)
...
def details(request, address):
    ...
    r[
        'trhead'
    ] = '<tr><th>Port</th><th style="width:300px;">Product / Version</th><th>Extra Info</th><th>&nbsp;</th></tr>'
    for ik in o['host']:
        pel = 0
        ...
        if str(saddress) == address:
            ...
            if 'hostnames' in i and type(i['hostnames']) is dict:
                ...
                if 'hostname' in i['hostnames']:
                    ...
                    if type(i['hostnames']['hostname']) is list:
                        ...
                        hostname += (
                            '<span class="small grey-text"><b>'
                            + i['hostnames']['hostname']['@type']
                            + ':</b> '
                            + i['hostnames']['hostname']['@name']
                            + '</span><br>'
                        )
            ...
            scantitle = (
                request.session['scanfile'].replace('.xml', '').replace('_', ' ')
            )
            if re.search('^webmapsched\_[0-9\.]+', request.session['scanfile']):
                m = re.search(
                    '^webmapsched\_[0-9\.]+\_(.+)', request.session['scanfile']
                )
            ...
            if scanmd5 in labelhost:
                if addressmd5 in labelhost[scanmd5]:
                    ...
                    labelout = (
                        '<span id="hostlabel" style="margin-left:60px;margin-top:-24px;" class="rightlabel '
                        + labelcolor
                        + '">'
                        + html.escape(labelhost[scanmd5][addressmd5])
                        + '</span>'
                    )
            ...
            for pobj in i['ports']['port']:
                ...
                if 'service' in p:
                    if '@ostype' in p['service']:
                        oshtml = (
                            '<div style="font-family:monospace;padding:6px;margin:6px;border-left:solid #666 1px;"><sup style="border-bottom:solid #ccc 1px;">Operating System</sup><br>'
                            + html.escape(p['service']['@ostype'])
                            + '</div>'
                        )
                ...
                if 'script' in p:
                    if '@id' in p['script']:
                        if p['script']['@id'] != 'fingerprint-strings':
                            so += (
                                '<div style="word-wrap: break-word;word-break: break-all;padding:6px;margin-left:6px;border-left:solid #666 1px;max-width:300px;font-size:12px;color:#ccc;font-family:monospace;"><sup style="color:#999;border-bottom:solid #999 1px;">script output</sup><br><b>'
                                + html.escape(p['script']['@id'])
                                + '</b> '
                                + html.escape(p['script']['@output'])
                                + '</div>'
                            )
                    else:
                        for sosc in p['script']:
                            if '@id' in sosc:
                                if sosc['@id'] != 'fingerprint-strings':
                                    so += (
                                        '<div style="word-wrap: break-word;word-break: break-all;padding:6px;margin:6px;border-left:solid #666 1px;max-width:300px;font-size:12px;color:#ccc;font-family:monospace;"><sup style="color:#999;border-bottom:solid #999 1px;">script output</sup><br><b>'
                                        + html.escape(sosc['@id'])
                                        + '</b> '
                                        + html.escape(sosc['@output'])
                                        + '</div>'
                                    )
                ...
                if p['state']['@state'] == 'open':
                        ...
                        if 'cpe' in p['service']:
                            if type(p['service']['cpe']) is list:
                                for cpei in p['service']['cpe']:
                                    cpe += (
                                        '<div class="grey-text" style="font-family:monospace;font-size:12px;">'
                                        + html.escape(cpei)
                                        + '</div>'
                                    )
                            else:
                    ...
                    r['trhost'] += (
                        ...
                        + ' / '
                        + v
                        + '<br><span style="font-size:12px;color:#999;">State: '
                        + p['state']['@state']
                        + '<br>Reason: '
                        + p['state']['@reason']
                        + '</span></td>'
                        ...
                        + '<td><ul id="dropdown'
                        + str(pel)
                        + '" class="dropdown-content" style="min-width:300px;">'
                        + '    <li><a href="#!" class="btncpy" data-clipboard-text="curl -v -A \'Mozilla/5.0 (Windows NT 6.1; WOW64; rv:40.0) Gecko/20100101 Firefox/40.1\' -k \'http://'
                        + html.escape(address)
                        + ':'
                        + html.escape(p['@portid'])
                        + '\'">Copy as curl command</a></li>'
                        + '    <li><a href="#!" class="btncpy" data-clipboard-text="nikto -host \'http://'
                        + html.escape(address)
                        + ':'
                        + html.escape(p['@portid'])
                        + '\'">Copy as nikto command</a></li>'
                        + '    <li><a href="#!" class="btncpy" data-clipboard-text="telnet '
                        + html.escape(address)
                        + ' '
                        + html.escape(p['@portid'])
                        + '">Copy as telnet command</a></li>'
                        + '</ul><a class="dropdown-trigger btn blue right" href="#!" data-target="dropdown'
                        + str(pel)
                        + '"><i class="material-icons">arrow_drop_down</i></a> '
                        + '<button onclick="javascript:apiPortDetails(\''
                        + html.escape(address)
                        + '\',\''
                        + html.escape(p['@portid'])
                        + '\');" class="btn blue right"><i class="material-icons">receipt</i></button></td>'
                        + '</tr>'
                    )
                elif p['state']['@state'] == 'filtered':
                    if 'service' in p:
                    ...
                    r['trhost'] += (
                        '<tr><td><span class="new badge grey" data-badge-caption="">'
                        + p['@protocol']
                        + ' / '
                        + p['@portid']
                        + '</span><br>'
                        ...
                        + p['state']['@reason']
                        + '</td>'
                        + '<td><button onclick="javascript:apiPortDetails(\''
                        + html.escape(address)
                        + '\',\''
                        + html.escape(p['@portid'])
                        + '\');" class="btn blue right"><i class="material-icons">receipt</i></button></td></tr>'
                    )
                else:
                    if 'service' in p:
                        servicename = p['service']['@name']
                    ...
                    r['trhost'] += (
                        ...
                        + servicename
                        + '</span></td>'
                        + '<td colspan="2" style="color:#999;font-size:12px;">State: '
                        + p['state']['@state']
                        + '<br>Reason: '
                        + p['state']['@reason']
                        + '</td>'
    ...
    if scanmd5 in noteshost:
        if addressmd5 in noteshost[scanmd5]:
            ...
            r['notes'] = base64.b64decode(urllib.parse.unquote(notesb64)).decode(
                'ascii'
            )
        ...
        if addressmd5 in cvehost[scanmd5]:
            ...
            for i in cvejson:
                ...
                for cveobj in listcve:
                    ...
                    cveout += (
                        '<div id="'
                        + html.escape(cveobj['id'])
                        + '" style="line-height:28px;padding:10px;border-bottom:solid #666 1px;margin-top:10px;">'
                        + '    <span class="label red">'
                        + html.escape(cveobj['id'])
                        + '</span> '
                        + html.escape(cveobj['summary'])
                        + '<br><br>'
                        + '    <div class="small" style="line-height:20px;"><b>References:</b><br>'
                        + cverefout
                        + '</div>'
                        + cveexdbout
                        + '</div>'
    ...
    r['js'] = (
        ...
        + '    $("#scantitle").html("'
        + html.escape(request.session['scanfile'])
        + '");'
        ...
        + '    $("#detailspo").html(\'<center><h4><i class="fas fa-door-open green-text"></i> '
        + str(po)
        + '</h4><span class="small grey-text">OPEN PORTS</span></center>\');'
        + '    $("#detailspc").html(\'<center><h4><i class="fas fa-door-closed red-text"></i> '
        + str(pc)
        + '</h4><span class="small grey-text">CLOSED PORTS</span></center>\');'
        + '    $("#detailspf").html(\'<center><h4><i class="fas fa-filter grey-text"></i> '
        + str(pf)
        + '</h4><span class="small grey-text">FILTERED PORTS</span></center>\');'
        + '}); '
        + '</script>'
    )

    return render(request, 'nmap_portdetails.html', r)
...
def index(request, filterservice="", filterportid=""):
    ...
    # gitcmd = os.popen('cd nmapdashboard/nmapreport && git rev-parse --abbrev-ref HEAD')
    ...
    if 'scanfile' in request.session:
        ...
        for i in xmlfiles:
            ...
            if re.search('^webmapsched\_[0-9\.]+', i):
                ...
                filename = (
                    '<i class="fas fa-calendar-alt grey-text"></i> '
        ...
        return render(request, 'nmap_xmlfiles.html', r)
        ...
        if 'ports' in i and 'port' in i['ports']:
                ...
                if 'service' in p:
                    ...
                    if '@ostype' in p['service']:
                        if p['service']['@ostype'] in allostypelist:
                            allostypelist[p['service']['@ostype']] = (
            ...
            for s in ss:
                if filterservice != ss[s]:
                    services += (
                        '<a href="/report/service/' + ss[s] + '/">' + ss[s] + '</a>, '
                    )
                else:
                    services += (
                        '<span class="tmlabel" style="background-color:#ffcc00;color:#333;">'
                        + ss[s]
                        + '</span>, '
                    )
            ...
            for oty in ost:
                ostype += (
                    '<i class="'
                    + fromOSTypeToFontAwesome(html.escape(ost[oty]))
                    + '"></i> <span class="grey-text small">'
                    + ost[oty].lower()
                    + '</span> '
                )
            ...
            for kp in pp:
                if filterportid != pp[kp]:
                    tdports += (
                        '<a href="/report/portid/' + pp[kp] + '/">' + pp[kp] + '</a>, '
                    )
                else:
                    tdports += (
                        '<span class="tmlabel" style="background-color:#ffcc00;color:#333;">'
            ...
            if scanmd5 in labelhost:
                if addressmd5 in labelhost[scanmd5]:
                    ...
                    newlabelout = (
                        '<div id="hostlabel'
                        + str(hostindex)
                        + '" style="z-index:99;transform: rotate(-8deg);margin-top:-14px;margin-left:-40px;" class="leftlabel '
                        + labelcolor
                        + '">'
                        + html.escape(labelhost[scanmd5][addressmd5])
                        + '</div>'
                        ...
                        + '" class="'
                        + labelcolor
                        + '" style="border-radius:0px 4px 0px 4px;z-index:98;position:absolute;width:18px;height:10px;margin-left:-54px;margin-top:-3px;"></div>'
                    )
            ...
            if scanmd5 in noteshost:
                if addressmd5 in noteshost[scanmd5]:
                    ...
                    notesout = (
                        '<a id="noteshost'
                        + str(hostindex)
                        + '" class="grey-text" href="#!" onclick="javascript:openNotes(\''
                        + hashlib.md5(str(address).encode('utf-8')).hexdigest()
                        + '\', \''
                        + notesb64
                        + '\');"><i class="fas fa-comment"></i> contains notes</a>'
                    )
                    removenotes = (
                        '<li><a href="#!" class="grey-text" onclick="javascript:removeNotes(\''
                        + addressmd5
                        + '\', \''
                        + str(hostindex)
                        + '\');">Remove notes</a></li>'
                ...
                if addressmd5 in cvehost[scanmd5]:
                    ...
                    if cvecount > 0:
                        cveout = (
                            '<a href="/report/'
                            + address
                            + '" class="grey-text"><i class="fas fa-exclamation-triangle"></i> '
                            + str(cvecount)
                            + ' CVE found</a>'
                        )

            if (
                (filterservice != "" and striggered is True)
                or (filterportid != "" and striggered is True)
                or (filterservice == "" and filterportid == "")
            ):
                portstateout = (
                    '<div style="overflow:none;background-color:#444;" class="tooltipped" data-position="top" data-tooltip="'
                    + str(po)
                    + ' open, '
                    + str(pc)
                    + ' closed, '
                    + str(pf)
                    + ' filtered">'
                    + '        <div class="perco" data-po="'
                    + str(po)
                    + '" style="padding-left:16px;padding-right:20px;"><b>'
                    + str(po)
                    + '</b></div>'
                    + ' </div>'
                )
    ...
    if filterservice == "" and filterportid == "":
        ...
        scaninfobox2 = (
            '<div class="small">'
            + '    <b class="orange-text">Filter port / service:</b> <b>'
            + html.escape(filterportid + filterservice)
            + '</b> <a href="/"><i class="fas fa-trash-alt"></i></a><br>'
            + '    <b class="orange-text">Total Ports:</b> '
            + str(totports)
            + '<br>'
            + '    <b class="orange-text">Open Ports:</b> '
            + str(ports['open'])
            + '<br>'
            + '    <b class="orange-text">Closed Ports:</b> '
            + str(ports['closed'])
            + '<br>'
            + '    <b class="orange-text">Filtered Ports:</b> '
            + str(ports['filtered'])
            + '</div>'
        )
    ...
    for i in sorted(picount, key=picount.__getitem__, reverse=True):
        if allpic <= 5:
            allpilinks += (
                '<a href="/report/portid/' + str(i) + '/">' + str(i) + '</a>, '
            )
    ...
    if filterservice == "" and filterportid == "":
        r['js'] += (
            ...
            + '            type: "doughnut", data: {labels:["Open", "Filtered", "Closed"], datasets: [{ data: ['
            + str(ports['open'])
            + ','
            + str(ports['filtered'])
            + ','
            + str(ports['closed'])
            + '], backgroundColor:["rgba(0,150,0,0.8)","rgba(255,200,0,0.8)","rgba(255,0,0,0.8)"], borderColor:"#ccc", borderWidth:0 }]}, options: {legend: { position: "right", labels: { fontColor: "#ccc" }  }}'
            + '        });'
            ...
            + '], datasets: [{ data: ['
            + allpidata[0:-1]
            + '], borderColor: "#fff", borderWidth:0,  backgroundColor:["#e6194b", "#3cb44b", "#ffe119", "#4363d8", "#f58231", "#911eb4", "#46f0f0", "#f032e6", "#bcf60c", "#fabebe", "#008080", "#e6beff", "#9a6324", "#fffac8", "#800000", "#aaffc3", "#808000", "#ffd8b1", "#000075", "#808080", "#ffffff", "#000000"] }]}, options: {legend: { position: "right", labels: { fontColor: "#ccc" }}}'
            + '        });'
            + '        var ctx = document.getElementById("chart2").getContext("2d");'
            + '        var myChart = new Chart(ctx, {'
            + '            type: "horizontalBar", data: { labels:['
            + allsslabels[0:-2]
            + '], datasets: [{ data: ['
            + allssdata[0:-1]
            + '], backgroundColor: "rgba(0,140,220,0.8)" }]}, options: {legend: { display: false }, scales: { xAxes: [{ ticks: { beginAtZero: true, fontColor: "#666" } }], yAxes: [{ ticks: { fontColor: "#666" } }] }  }'
            + '        });'
            + '    });'
            + '</script>'
        )
    ...
    r['js'] += (
        ...
        + '        $(".tooltipped").tooltip();'
        + '        $(".perco").each(function() { '
        + '            var pwidth = ( (($(this).attr("data-po") * 100) / '
        + str(totports)
        + ') ); '
        + '            /* console.log(pwidth); */ '
        ...
        + '        });'
        + '    $("#detailstopports").html(\'<span class="small">'
        + str(allss[0:-2])
        + '</span>\');'
        + '    });'
        + '</script>'
    ...
    r['cpestring'] = (
        ' <input type="hidden" id="cpestring" value="'
        + urllib.parse.quote_plus(base64.b64encode(json.dumps(cpedict).encode()))
        + '" /> '
    )

    return render(request, 'nmap_hostdetails.html', r)
...
def scan_diff(request, f1, f2):
    ...
    return render(request, 'nmap_ndiff.html', r)
...
def about(request):
    ...
    return render(request, 'nmap_about.html', r)
