...
def rmNotes(request, hashstr):
    ...
    scanfilemd5 = hashlib.md5(
        str(request.session['scanfile']).encode('utf-8')
    ).hexdigest()
...
def saveNotes(request):
    ...
    if request.method == "POST":
        ...
        if re.match('^[a-f0-9]{32,32}$', request.POST['hashstr']) is not None:
            f = open(
                '/opt/notes/' + scanfilemd5 + '_' + request.POST['hashstr'] + '.notes',
                'w',
            )
...
def rmlabel(request, objtype, hashstr):
    ...
    if re.match('^[a-f0-9]{32,32}$', hashstr) is not None:
        os.remove(
            '/opt/notes/' + scanfilemd5 + '_' + hashstr + '.' + objtype + '.label'
        )
    ...
    for ik in o['host']:
        ...
        if str(saddress) == address:
            for pobj in i['ports']['port']:
                ...
                if p['@portid'] == portid:
                    return HttpResponse(
                        json.dumps(p, indent=4), content_type="application/json"
                    )
...
def genPDF(request):
    ...
    if 'scanfile' in request.session:
        ...
        os.popen(
            '/opt/wkhtmltox/bin/wkhtmltopdf --cookie sessionid '
            + request.session._session_key
            + ' --enable-javascript --javascript-delay 6000 http://127.0.0.1:8000/view/pdf/ /opt/nmapdashboard/nmapreport/static/'
            + pdffile
            + '.pdf'
        )
...
def getCVE(request):
    ...
    if request.method == "POST":
        ...
        cveproc = os.popen(
            'python3 /opt/nmapdashboard/nmapreport/nmap/cve.py '
        ...
        cpe = json.loads(
            base64.b64decode(urllib.parse.unquote(request.POST['cpe'])).decode('ascii')
        )
...
def apiv1_scan(request):
    ...
    gitcmd = os.popen(
        'cd /opt/nmapdashboard/nmapreport && git rev-parse --abbrev-ref HEAD'
    )
