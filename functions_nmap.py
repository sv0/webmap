        ...
        if re.search('\.xml.active$', ff) is not None:
            ...
            with open('/tmp/' + ff) as n:
                ...
                for line in lines:
                    ...
                    rx = re.search(
                        'args\=.+\-oX \/tmp\/(.+\.xml).+ start\=.+ startstr\=.(.+). version\=',
                        line.strip(),
                    )
                    if rx is not None:
                    ...
                    rx = re.search(
                        'scaninfo type\=.(.+). protocol\=.(.+). numservices',
                        line.strip(),
                    )
                    if rx is not None:
...
def nmap_newscan(request):
    if request.method == "POST":
        if (
            re.search('^[a-zA-Z0-9\_\-\.]+$', request.POST['filename'])
            and re.search('^[a-zA-Z0-9\-\.\:\=\s,]+$', request.POST['params'])
            and re.search('^[a-zA-Z0-9\-\.\:\/\s]+$', request.POST['target'])
        ):
            res = {'p': request.POST}
            os.popen(
                'nmap '
            ...
            if request.POST['schedule'] == "true":
                ...
                filenamemd5 = hashlib.md5(
                    str(request.POST['filename']).encode('utf-8')
                ).hexdigest()
                writefile = (
                    settings.BASE_DIR
                    + '/nmapreport/nmap/schedule/'
                    + filenamemd5
                    + '.json'
                )
